--- Simplified utilities for typstwriter.nvim v2
...
--- @param doc_type string Document type from metadata
--- @return string Generated filename
function M.generate_filename(title, doc_type)
  -- Simple filename: type-title-YYYYMMDD.typ
  local date = os.date("%Y%m%d")
  local safe_title = title:gsub("[^%w%s%-]", ""):gsub("%s+", "-"):lower()
  return string.format("%s-%s-%s.typ", doc_type, safe_title, date)
end
...
function M.notify(message, level)
  if not v2_config.should_notify() then
    return
  end

  level = level or v2_config.get_notification_level()
  vim.notify("[TypstWriter v2] " .. message, level)
end
