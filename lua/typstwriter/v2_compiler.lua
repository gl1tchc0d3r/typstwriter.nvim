--- Simplified compilation workflow for typstwriter.nvim v2
...
function M.compile_current(file_path)
  ...
  if vim.fn.expand("%:e") ~= "typ" then
    v2_utils.notify("Not a Typst file", vim.log.levels.WARN)
    return false
  end
  ...
  -- Check file exists
  if not v2_utils.file_exists(file_path) then
    v2_utils.notify("File not found: " .. file_path, vim.log.levels.ERROR)
    return false
  end

  -- Validate metadata if required
  if v2_config.get("require_metadata") then
    local meta = metadata.parse_metadata(file_path)
    if not meta then
      v2_utils.notify("Document has no metadata. Add #metadata((...)) block.", vim.log.levels.WARN)
      return false
    end

    local valid, error_msg = v2_config.validate_metadata(meta)
    if not valid then
      v2_utils.notify("Invalid metadata: " .. error_msg, vim.log.levels.ERROR)
      return false
    end
  end
  ...
  local success = v2_utils.system_exec(cmd, "✓ Compiled to PDF", "✗ Compilation failed")
...
function M.open_pdf(file_path)
  file_path = file_path or vim.fn.expand("%:p")
  local pdf_file = v2_utils.get_pdf_path(file_path)

  if not v2_utils.file_exists(pdf_file) then
    v2_utils.notify("PDF not found: " .. vim.fn.fnamemodify(pdf_file, ":t"), vim.log.levels.WARN)
    return false
  end

  local opener = v2_utils.get_pdf_opener()
  if not opener then
    v2_utils.notify("No PDF opener available", vim.log.levels.ERROR)
    return false
  end
  ...
  if success then
    v2_utils.notify("Opened: " .. vim.fn.fnamemodify(pdf_file, ":t"))
  else
    v2_utils.notify("Failed to open PDF", vim.log.levels.ERROR)
  end
...
function M.get_status(file_path)
  ...
  local status = {
    ...
    pdf_file = pdf_file,
    typst_exists = v2_utils.file_exists(file_path),
    pdf_exists = v2_utils.file_exists(pdf_file),
    has_typst_binary = v2_utils.has_typst(),
    has_pdf_opener = v2_utils.get_pdf_opener() ~= nil,
    metadata = nil,
  ...
  if status.typst_exists then
    ...
    if status.metadata then
      local valid, error_msg = v2_config.validate_metadata(status.metadata)
...
function M.show_status()
  ...
  if status.metadata then
    ...
    print("  Has metadata:     ✗")
    print("  Note: Add #metadata((...)) block for v2 features")
  end
