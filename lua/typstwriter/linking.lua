...
function M.create_link_by_name(document_name, link_format)
  ...
  for _, doc in ipairs(documents) do
    if
      (doc.title and doc.title:lower() == document_name:lower())
      or (doc.basename:lower() == document_name:lower())
      or (doc.filename:lower() == document_name:lower())
    then
      found_doc = doc
      break
    end
  ...
  if found_doc then
    ...
    local confirm =
      vim.fn.confirm(string.format('Document "%s" not found. Create new document?', document_name), "&Yes\n&No", 1)
